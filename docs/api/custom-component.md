...
::: warning 高阶函数
与 `RootComponent` 类似，`CustomComponent` 也需调用两次：

```ts
// 第一次调用：传入泛型参数
// 第二次调用：传入配置选项
CustomComponent<RootDoc, CompDoc, "Prefix">()({/* options */});
```
...
- 相关类型：[CustomInheritConstraint](https://github.com/missannil/annil/blob/main/src/api/CustomComponent/CustomInherit/CustomInheritConstraint.ts) · [CustomEventsConstraint](https://github.com/missannil/annil/blob/main/src/api/CustomComponent/CustomEvents/CustomEventsConstraint.ts) · [ComponentDoc](../api/define-component.md)
