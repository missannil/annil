...
test("store数据初始化在attached周期", async () => {
  ...
  expect(comp.instance.data.chunk_age).toBe(20);
  expect(comp.instance.data.custom_age).toBe(20);
  storeUser.changeAge();
  // 2. store字段响应式更新
  expect(comp.instance.data.age).toBe(21);
  expect(comp.instance.data.chunk_age).toBe(21);
  ...
  expect(comp.instance.data.chunk_age).toBe(22);
  ...
  expect(comp.instance.data.chunk_age).toBe(22); // 这里不变，因为已经解绑了
