...
describe("watch-multipleFields", () => {
  ...
  test("监控字段为多个属性", async () => {
    ...
    expect(comp.data.watchValues).toStrictEqual([123, 20, "zhao", 123, undefined, "zhao"]);
    ...
    expect(comp.data.watchCount).toStrictEqual(4);
    expect(comp.data.watchValues).toStrictEqual([456, 30, "zhao", 456, 20, "zhao"]);

    comp.setData({
      "obj.name": "lili",
    });
    expect(comp.data.watchCount).toStrictEqual(5);
